...
const Solution = () => {
  return (
    <>
      <div className="bg-[#F0F0F0] min-h-screen w-full relative mt-6 mb-6">
        {/* Height Changed for lg & md 4xl screen. */}
        <div className="bg-solution-gradient h-[50vh] sm:h-[50vh] md:h-[50vh] lg:h-[50vh] xl:h-[50vh] 2xl:h-[50vh] 4xl:h-[50vh]">
          {/* Top Section - Heading */}
          <div className="absolute top-0 left-10 text-left py-16">
            {/* Text Size Changed for lg, md & sm screen size. */}
            <h1 className="text-white text-4xl 4xl:text-5xl 3xl:text-4xl 2xl:text-4xl xl:text-4xl lg:text-4xl md:text-3xl sm:text-2xl xs:text-2xl font-bold">
              SOLUTIONS
            </h1>
          </div>
          ...
          <Swiper
            ...
            // navigation
            // Added width for sm & 
            className="absolute top-[30%] left-1/2 transform -translate-x-1/2 z-10 w-11/12 md:w-3/4 sm:w-3/5 lg:w-1/2 4xl:w-1/2 h-[50vh] xs:h-[50vh]  sm:h-[30vh] md:h-[40vh] lg:h-[50vh] xl:h-[50vh] 2xl:h-[30vh] 4xl:h-[40vh]"
          > 
            {cardsData.map((card) => (
              <SwiperSlide key={card.id}>
                <div className="bg-[#FFFFFF] opacity-90 shadow-lg rounded-lg p-8 sm:p-6 xs:p-4 flex flex-col items-center justify-center text-center">
                  <h2 className="bg-gradient-to-r from-[#0048FF] to-[#D6008D] bg-clip-text text-transparent font-bold text-2xl xs:text-base sm:text-lg md:text-3xl mb-4 4xl:text-5xl">
                    {card.title}
                  </h2>
